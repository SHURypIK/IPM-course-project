...
public class PositionRepo {
    ...
    private final static String UpdateURL = "http://localhost:8080/position/update/";
    private final static String GetURL = "http://localhost:8080/position/all";
    private final static String DeleteURL = "http://localhost:8080/position/delete/";
    private final static String AddURL = "http://localhost:8080/position/add";


    public static Position add(Position position) throws Exception {
        Gson gson = new Gson();

        String jsonBody = gson.toJson(position);
            ...
            try (CloseableHttpResponse response = client.execute(httpPost)) {
                ...
                if (statusCode == HttpStatus.SC_OK) {
                    ...
                    if (jsonElement.isJsonObject()) {
                        Position position1 = gson.fromJson(jsonElement, Position.class);
                        return position1;
    ...
    public static Position update(Position position, int id) throws Exception {
        Gson gson = new Gson();
    ...
    public static ArrayList<Position> get() throws Exception {
            ...
            try (CloseableHttpResponse response = client.execute(httpGet)) {
                ...
                if (statusCode == HttpStatus.SC_OK) {
                    ...
                    if (jsonElement.isJsonArray()) {
                        ArrayList<Position> positions = gson.fromJson(jsonElement, positionListType);
                        return positions;
                    } else if (jsonElement.isJsonPrimitive() && jsonElement.getAsJsonPrimitive().isString()) {
                        String responseString = jsonElement.getAsString();
                        throw new Exception(responseString);
