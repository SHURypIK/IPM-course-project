...
public class ContractRepo {
    ...
    private final static String UpdateURL = "http://localhost:8080/lease_contract/update/";
    private final static String DeleteURL = "http://localhost:8080/lease_contract/delete/";
    private final static String AddURL = "http://localhost:8080/lease_contract/add";


    public static LeaseContract add(LeaseContract contract) throws Exception {
        Gson gson = new GsonBuilder().registerTypeAdapter(LocalDate.class, new LocalDateAdapter()).create();

        contract.setTenantFIO(AppState.getInstance().getResident().getFio());
        String jsonBody = gson.toJson(contract);
            ...
            try (CloseableHttpResponse response = client.execute(httpPost)) {
                ...
                if (statusCode == HttpStatus.SC_OK) {
                    ...
                    if (jsonElement.isJsonObject()) {
                        LeaseContract contract1 = gson.fromJson(jsonElement, LeaseContract.class);
                        return contract1;
    ...
    public static LeaseContract update(LeaseContract contract, int id) throws Exception {
        Gson gson = new GsonBuilder().registerTypeAdapter(LocalDate.class, new LocalDateAdapter()).create();
